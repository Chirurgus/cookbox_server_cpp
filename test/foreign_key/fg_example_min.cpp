...
int main() {
 ...
 /*   
    ...
    storage.replace(Track{ 11, "That's Amore", 1 });
    storage.replace(Track{ 12, "Christmas Blues", 1 });
    storage.replace(Track{ 13, "My Way", 2 });
    
    try{
        //  This fails because value inserted into the trackartist column (3)
        //  does not correspond to row in the artist table.
        storage.replace(Track{ 14, "Mr. Bojangles", 3 });
        assert(0);
...
*/
   /* 
    ...
    storage.replace(Track{ 15, "Boogie Woogie", 3 });
